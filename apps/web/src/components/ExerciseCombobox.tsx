...
export function ExerciseCombobox({
  ...
  return (
    <div className="exercise-combobox" ref={containerRef}>
      ...
      />
      {isOpen && (
        <ul className="exercise-combobox__listbox" role="listbox" id={listboxId} aria-label="Esercizi">
          {flatItems.length === 0 && <li className="exercise-combobox__empty">Nessun esercizio trovato.</li>}
          {(() => {
            let index = -1;
            return filtered.map(([muscleGroup, items]) => (
              <li key={muscleGroup} className="exercise-combobox__group" role="group" aria-label={muscleGroup}>

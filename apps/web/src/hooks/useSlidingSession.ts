 ...
 * montato: pensato per pagine dove un'attivita' puo' durare piu' a lungo
 * della scadenza fissa del token (1h) — oggi solo Registra sessione, dove
 * una sessione di allenamento puo' superare abbondantemente un'ora. Il
 * resto dell'app mantiene il comportamento normale (logout automatico dopo
 * un'ora di inattivita', vedi AuthProvider): e' una scelta di sicurezza
 * voluta, non un'omissione — questo hook va richiamato solo dalle pagine
 * dove serve davvero restare loggati piu' a lungo.
 *
 ...
 * l'intera ora, es. rete assente), la prossima richiesta autenticata
 * fallira' con 401 e AuthProvider fara' il consueto logout automatico —
 * stesso fallback di sempre, non un caso nuovo da gestire qui.
 */

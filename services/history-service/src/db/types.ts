...
/**
 * Tipi delle tabelle per Kysely (unica fonte di verita' lato tipi).
 *
 * `workout_sessions`/`session_sets` NON hanno una migrazione di creazione in
 * questo servizio: esistono gia' in Postgres, create dalle migrazioni
 * storiche di progress-service prima dello split dei due servizi (vedi
 * db/migrate.ts). Queste interfacce devono restare allineate a quello
 * schema fisico, non a migrazioni proprie di history-service.
 *
 * Nota: le colonne `numeric` (progression_increment, actual_weight) tornano
 * da node-pg come stringa, quindi in select sono `string | null`; in
 * insert/update accettiamo `number | null` (stesso trattamento di
 * target_weight in workout-service).
 */
